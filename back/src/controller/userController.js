...
exports.signup = async function (req, res) {
    ...
    // 중복 회원 검사 (중복된 회원이 있으면 1개 row가 반환될 것임)
...
exports.signin = async function (req, res) {
    const { email, password } = req.body; // 배열의 비구조할당은 대괄호, 객체(req는 객체)의 비구조할당은 중괄호 사용
    ...
    const isValidUser = await userDao.selectUser(email, password);
            ...
            //테스트용 구문 return res.send(isValidUser);
    ...
    if (isValidUser.length < 1) {
        return res.send({
            ...
            message: "존재하지 않는 회원입니다."
             ...
             // 테스트용 구문   console.log(isValidUser);
    ...
    const token = jwt.sign(
        { userIdx: userIdx },   // 페이로드 (주로 userIdx값을 넣음, 여러 API요청시 누구인지 알 수 있기때문)
        jwtSecret       // 시크릿 키
    );
...
exports.getNicknameByToken = async function(req, res) {
    ...
    const [userInfo] = await userDao.selectNicknmeByUserIdx(userIdx);
